...
def generate_dataparser_outputs(
    ...
    if build_pointcloud:
        ...
        for i, frame in tqdm(enumerate(range(start_frame, end_frame+1))):
            ...
            points_xyz_world = points_xyz_vehicle @ ego_pose.T ##yichu
            ...
            # filer points in tracking bbox # yichu
            ...
            for tracklet in object_tracklets_vehicle[i]:
                ...
                if track_id >= 0:
                    ...
                    points_xyz_obj = points_xyz_vehicle @ vehicle2local.T # yichu
...
def generate_shifting_dataparser_outputs(
        datadir,
        generative_datadir,
        selected_frames=None,
        build_pointcloud=True, 
    ...
    image_dir = os.path.join(datadir, 'images')
    image_filenames_all = sorted(glob(os.path.join(image_dir, '*.png')))
    num_frames_all = len(image_filenames_all) // len(cameras)
    num_cameras = len(cameras)
    ...
    for image_filename in image_filenames_all:
        ...
        if frame >= start_frame and frame <= end_frame and cam in cameras:
            ...
            # frames.append(frame)
            # frames_idx.append(frame - start_frame)
            # cams.append(cam + len(cfg.data.get('cameras', [0, 1, 2])))
            frames.append(frame + idx_start)
            frames_idx.append(frame - start_frame + idx_start)
            cams.append(cam)
            image_filenames.append(os.path.join(generative_datadir, image_basename))
    ...
    # get object bounding mask
    obj_bounds = [None] * len(image_filenames)
    result['obj_bounds'] = obj_bounds
